...
include_once('copyttt2Service.php');
...
alog("Copyttt2Control___________________________start");
...
}else if($objAuth->isAuth("COPYTTT2",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"COPYTTT2",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"COPYTTT2",$ctl,"N");
...
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new copyttt2Service();
	//컨트롤 명령별 분개처리
alog("ctl:" . $ctl);
switch ($ctl){
			default:
...
alog("Copyttt2Control___________________________end");
