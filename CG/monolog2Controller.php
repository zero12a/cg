...
include_once('monolog2Service.php');
...
$log->info("Monolog2Control___________________________start");
...
}else if($objAuth->isAuth("MONOLOG2",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"MONOLOG2",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"MONOLOG2",$ctl,"N");
...
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new monolog2Service();
	//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
			case "G1_SEARCHALL" :
...
$log->info("Monolog2Control___________________________end");
$log->close(); unset($log);
?>
