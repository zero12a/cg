...
include_once('copyttt8Service.php');
...
$log->info("Copyttt8Control___________________________start");
...
}else if($objAuth->isAuth("COPYTTT8",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"COPYTTT8",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"COPYTTT8",$ctl,"N");
...
$REQ["G2-CHK"] = filterGridChk($REQ["G2-CHK"],"STRING",30,"CLEARTEXT","/--미 정의--/");//PCD 입력값검증
	...
	//서비스 클래스 생성
$objService = new copyttt8Service();
	//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
			case "G1_SEARCHALL" :
...
$log->info("Copyttt8Control___________________________end");
$log->close(); unset($log);
?>
