...
include_once('copyttt7Service.php');
...
alog("Copyttt7Control___________________________start");
...
}else if($objAuth->isAuth("COPYTTT7",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"COPYTTT7",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"COPYTTT7",$ctl,"N");
...
$REQ["G2-CHK"] = filterGridChk($REQ["G2-CHK"],"STRING",30,"CLEARTEXT","/--미 정의--/");//PCD 입력값검증
	...
	//서비스 클래스 생성
$objService = new copyttt7Service();
	//컨트롤 명령별 분개처리
alog("ctl:" . $ctl);
switch ($ctl){
			case "G1_SEARCHALL" :
...
alog("Copyttt7Control___________________________end");
