...
require_once('pisql2bbService.php');
...
$log->info("Pisql2bbControl___________________________start");
...
}else if($objAuth->isAuth("PISQL2bb",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"PISQL2bb",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"PISQL2bb",$ctl,"N");
...
//,  입력값 필터 
	...
	//서비스 클래스 생성
$objService = new pisql2bbService();
	//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
		case "G1_SEARCHALL" :
	...
	case "G2_CHKSAVE" :
  		...
  		break;
	case "G3_SEARCH" :
  		echo $objService->goG3Search(); //1, 조회
  		break;
	case "G3_SAVE" :
  		echo $objService->goG3Save(); //1, 저장
  		break;
	case "G3_DELETE" :
  		echo $objService->goG3Delete(); //1, 삭제
  		break;
	default:
...
$log->info("Pisql2bbControl___________________________end");
$log->close(); unset($log);
?>
