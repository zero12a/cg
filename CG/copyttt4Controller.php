...
include_once('copyttt4Service.php');
...
alog("Copyttt4Control___________________________start");
...
}else if($objAuth->isAuth("COPYTTT4",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"COPYTTT4",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"COPYTTT4",$ctl,"N");
...
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new copyttt4Service();
	//컨트롤 명령별 분개처리
alog("ctl:" . $ctl);
switch ($ctl){
			default:
...
alog("Copyttt4Control___________________________end");
