...
class cg_pgminfo_svc 
{
	...
	*/
	...
	public function goIoSave(){
		...
		$grpId="IO";
		$GRID["XML"]=$REQ[$grpId."-XML"];
		$GRID["COLORD"] = "PJTSEQ,PGMSEQ,GRPSEQ,IOSEQ,DDSEQ,COLID,COLORD,COLNM,DATATYPE,VALIDSEQ,DATASIZE,OBJTYPE,PROPERTY,POPUP,BRYN,LBLHIDDENYN,LBLWIDTH,LBLALIGN,OBJWIDTH,OBJHEIGHT,OBJALIGN,KEYYN,SEQYN,HIDDENYN,EDITYN,FNINIT,FNCHANGE,FORMAT,FOOTERNM,FOOTERMATH,ICONNM,ICONSTYLE,LBLSTYLE,OBJSTYLE,OBJ2STYLE,PLACEHOLDER,BTNHIDDENYN,FILTER,ADDDT,MODDT"; //그리드 컬럼순서(Hidden컬럼포함)
	...
	*/
	...
	public function goGrpSave(){
		...
		$grpId="GRP";
		$GRID["XML"]=$REQ[$grpId."-XML"];
		$GRID["COLORD"] = "PJTSEQ,PGMSEQ,GRPSEQ,GRPID,GRPTYPE,GRPNM,GRPORD,FREEZECNT,REFGRPID,VBOX,GRPWIDTH,GRPHEIGHT,COLSIZETYPE,LEGENDALIGN,STACKED,PROPERTY,METHOD,KEYCOLID,SEQYN,ADDDT,MODDT"; //그리드 컬럼순서(Hidden컬럼포함)
		//암호화컬럼
		$GRID["COLCRYPT"] = array();	
