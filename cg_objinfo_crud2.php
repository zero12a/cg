...
if($REQ["F_GRPID"] == "2" && $REQ["G2_CRUD_MODE"] == "read"){
    ...
    $sql = "
          select
             OBJDSEQ,OBJTYPE,FILETYPE,OBJVAL,OBJDORD,OBJVALTYPE,OBJVALNM,OBJDESC,SRCTXT,SPTTXT,INPUT,PARAM,SRCTYPE,FILTER,ADDDT,MODDT
    ...
    $sql_inserted = "
               insert into CG_OBJINFOD (
									...
									,INPUT,PARAM,SRCTYPE,FILTER
                                    ,ADDDT
               ) values (
                                    #F_PJTID#, #OBJTYPE#,#FILETYPE#,#OBJVAL#,#OBJDORD#
									,#OBJVALTYPE#,#OBJVALNM#,#OBJDESC#,#SRCTXT#,#SPTTXT#
									,#INPUT#,#PARAM#,#SRCTYPE#,#FILTER#
                                    ,date_format(sysdate(),'%Y%m%d%H%i%s')
               )
    ";
    $sql_inserted_coltype = "ssssi sssss ssss";
    ...
    $sql_updated = "
               update CG_OBJINFOD set
					...
					, PARAM = #PARAM#, SRCTYPE = #SRCTYPE#, FILTER = #FILTER#
                    ,MODDT = date_format(sysdate(),'%Y%m%d%H%i%s')
                where PJTID = #F_PJTID# and OBJDSEQ = #OBJDSEQ#
    ";
    $sql_updated_coltype = "ssssi sssss sss si";
...
if($REQ["F_GRPID"] == "3" && $REQ["G3_CRUD_MODE"] == "read"){
    ...
    $sql = "
          select
            OBJASEQ,OBJTYPE,OBJDSEQ,OBJAORD,OBJDESC,SRCTXT,SPTTXT,INPUT,PARAM,SRCTYPE,FILTER,ADDDT,MODDT
    ...
    $sql_inserted = "
               insert into CG_OBJINFOA (
                                    ...
                                    #F_PJTID#,#OBJTYPE#,#OBJDSEQ#,#OBJAORD#,#OBJDESC#
                                    ,#SRCTXT#,#SPTTXT#,#INPUT#,#PARAM#,#SRCTYPE#
									,#FILTER#
                                    ,date_format(sysdate(),'%Y%m%d%H%i%s')
               )
    ";
    $sql_inserted_coltype = "ssiis sssss s";

    $sql_deleted = " delete from CG_OBJINFOA where PJTID = #F_PJTID# and OBJASEQ = #OBJASEQ# ";
    $sql_deleted_coltype = "si";

    $sql_updated = "
              update CG_OBJINFOA set
                    OBJTYPE = #OBJTYPE#, OBJDESC = #OBJDESC#, OBJAORD = #OBJAORD#, SRCTXT = #SRCTXT#, SPTTXT = #SPTTXT#
					, INPUT = #INPUT#, PARAM = #PARAM#, SRCTYPE = #SRCTYPE#, FILTER = #FILTER#
                    ,MODDT = date_format(sysdate(),'%Y%m%d%H%i%s')
                where PJTID = #F_PJTID# and OBJASEQ = #OBJASEQ#
    ";
    $sql_updated_coltype = "ssiss ssss si";


    echo make_grid_save_json($db,$REQ,$colord,$xml_array,$sql_inserted,$sql_inserted_coltype,$sql_deleted,$sql_deleted_coltype,$sql_updated,$sql_updated_coltype,"Y","OBJASEQ");
...
if($REQ["F_GRPID"] == "5" && $REQ["G5_CRUD_MODE"] == "read"){
    ...
    $sql = "
          select
            OBJBSEQ,OBJTYPE,OBJASEQ,OBJBORD,OBJDESC,SRCTXT,SPTTXT,INPUT,PARAM,SRCTYPE,FILTER,ADDDT,MODDT
          from CG_OBJINFOB where PJTID = #F_PJTID# and OBJASEQ = #G3_OBJASEQ#
    ...
    $sql_inserted = "
               insert into CG_OBJINFOB (
                                    PJTID,OBJTYPE,OBJASEQ,OBJBORD,OBJDESC
                                    ,SRCTXT,SPTTXT,INPUT,PARAM,SRCTYPE
									,FILTER
                                    ,ADDDT
               ) values (
                                    #F_PJTID#,#OBJTYPE#,#OBJASEQ#,#OBJBORD#,#OBJDESC#
                                    ,#SRCTXT#,#SPTTXT#,#INPUT#,#PARAM#,#SRCTYPE#
									,#FILTER#
                                    ,date_format(sysdate(),'%Y%m%d%H%i%s')
               )
    ...
    $sql_deleted = " delete from CG_OBJINFOB where PJTID = #F_PJTID# and OBJBSEQ = #OBJBSEQ# ";
    $sql_deleted_coltype = "si";
    ...
    $sql_updated = "
              update CG_OBJINFOB set
					...
					, INPUT = #INPUT#, PARAM = #PARAM#, SRCTYPE = #SRCTYPE#, FILTER = #FILTER#
                    ,MODDT = date_format(sysdate(),'%Y%m%d%H%i%s')
                where PJTID = #F_PJTID# and OBJBSEQ = #OBJBSEQ#
    ";
    $sql_updated_coltype = "ssiss ssss si";
