...
<style>
    ...
    //그리드 조회(VALID)
    function gridSearch8(tinput){
        alog("gridSearch8()------------start");
        ...
        $.ajax({
            type : "POST",
            url : mygrid12_url+"&G12_CRUD_MODE=read&" + tinput ,
            data : {xmldata : ""},
            dataType: "json",
            async: true,
            success: function(data){
                alog("   gridSearch8 json return----------------------");
                alog("   json data : " + data);
                alog("   json RTN_CD : " + data.RTN_CD);
                alog("   json ERR_CD : " + data.ERR_CD);
                //alog("   json RTN_MSG length : " + data.RTN_MSG.length);
                ...
                if(data.RTN_CD == "200"){
					...
					msgNotice("[VALID] 조회 성공했습니다. ("+row_cnt+"건)",1);

                }else{
                    msgError("[VALID] 서버 조회중 에러가 발생했습니다.\nRTN_CD : " + data.RTN_CD + "\nERR_CD : " + data.ERR_CD + "\nRTN_MSG :" + data.RTN_MSG,3);
                }
            },
            error: function(error){
    ...
    function initBody(){
		...
		//8번째 그리드 초기화(VALID)
		alog("grid8-----------------------------start");
		mygrid8 = new dhtmlXGridObject('grid8');
		mygrid8.setUserData("","gridTitle","grid8 : sql column list"); //글로별 변수에 그리드 타이블 넣기
		mygrid8.setImagePath("./lib/dhtmlxSuite/codebase/imgs/");
		mygrid8.setHeader("SEQ,PJT,PGM,GRP,FNC,ORD,TIME,JOB,COLID,VAL1,VAL2,OPER,ANDOR,ACT,MSG,SQL,ADDDT,MODDT");
		mygrid8.setColumnIds("VALIDSEQ,PJTID,PGMID,GRPID,FNCID,ORD,TIMETYPE,JOBTYPE,COLID,VAL1,VAL2,OPER,ANDOR,ACTION,MSG,SQLTXT,ADDDT,MODDT");
		mygrid8.setInitWidths("50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50");
		mygrid8.setColTypes("ro,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ro,ro");
		mygrid8.setColAlign("left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left,left");
		mygrid8.setColSorting("int,str,str,str,str,int,str,str,str,str,str,str,str,str,str,str,str,str");


		mygrid8.enableSmartRendering(false);
		mygrid8.enableMultiselect(true);
		mygrid8.init();

		//mygrid8.setColumnHidden(0,true); //VALIDSEQ
		//mygrid8.setColumnHidden(1,true); //PJTID
		//mygrid8.setColumnHidden(2,true); //PGMID
		//mygrid8.setColumnHidden(3,true); //SQLID

		mygrid8.attachEvent("onEditCell", function(stage,rId,cInd,nValue,oValue){

			//alog("mygrid8  onEditCell ------------------start");
			//alog("       stage : " + stage);
			//alog("       rId : " + rId);
			//alog("       cInd : " + cInd);
			//alog("       nValue : " + nValue);
			...
			RowEditStatus = mygrid8.getUserData(rId,"!nativeeditor_status");
			if(stage == 2
				&& RowEditStatus != "inserted"
				&& RowEditStatus != "deleted"
				&& nValue != oValue
				...
				if(RowEditStatus == "") {
					mygrid8.setUserData(rId,"!nativeeditor_status","updated");
					mygrid8.setRowTextBold(rId);
				}
				mygrid8.cells(rId,cInd).cell.wasChanged = true;
			}
			return true;
...
<div id="BODY_BOX" class="BODY_BOX">
    ...
    <div class="GRP_LINE" style="">
        ...
        <div class="GRP_OBJECT" style="width:20%;">
            <div  class="GRID_LABELGRP" >
                <div class="GRID_LABEL" >* VALID</div>
                <div  class="GRID_LABELBTN"  >
				<input type="button" name="some_name" value="S8" onclick="save8();">
                <input type="button" name="add" value="+" onclick="addRow8();">
                <input type="button" name="delete" value="-" onclick="delRow(mygrid8);">
                </div>
            </div>

            <div class="GRID_OBJECT" >
                <div id="grid8" width="100%" height="200px" style="background-color:white;z-index:3;"></div>
            </div>
        </div>
