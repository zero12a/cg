//글로벌 변수 선언	
//버틀 그룹쪽에서 컨틀롤러 호출
var url_G1_SEARCHALL = "findfooterController?CTLGRP=G1&CTLFNC=SEARCHALL";
// 변수 선언	
var obj_G1_EX_TEAM_NM; // 그래프 제외 팀명 변수선언
//컨트롤러 경로
  ...
  alog("G3_INIT()-------------------------start");
	...
	//가로 정렬	
		...
		mygridG3.attachFooter(
			"<div id='G3-UUID_SEQ_SUM'>-</div>,<div id='G3-TEAM_NM_SUM'>합계</div>,<div id='G3-PRIORITY_1_SUM'></div>,<div id='G3-PRIORITY_2_SUM'></div>,<div id='G3-PRIORITY_3_SUM'></div>,<div id='G3-VUL_CNT_SUM'></div>",
    ...
    function G3_SEARCH(tinput,token){
        ...
        $.ajax({
            ...
            error: function(error){
                ...
                alog("[팀별 현황 (보안취약점 갯수)2] Ajax http 500 error ( " + data.RTN_MSG + " )");
            }
        });
        alog("G3_SEARCH()------------end");
    }
