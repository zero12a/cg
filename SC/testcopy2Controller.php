...
include_once('testcopy2Service.php');
...
alog("Testcopy2Control___________________________start");
...
}else if($objAuth->isAuth("TESTCOPY2",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"TESTCOPY2",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"TESTCOPY2",$ctl,"N");
...
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new testcopy2Service();
	//컨트롤 명령별 분개처리
alog("ctl:" . $ctl);
switch ($ctl){
			case "G1_SEARCHALL" :
...
alog("Testcopy2Control___________________________end");
