<?php
    ...
    //그룹ID받기
    $REQ["F_TBLNM"] = $_GET['F_TBLNM'];
    $REQ["F_COLID"] = $_GET['F_COLID'];
    $REQ["F_COLNM"] = $_GET['F_COLNM'];
...
if($F_GRPID == "1" && $G1_CRUD_MODE == "read"){
    ...
    $sql = "
          select
            TBLNM,COLID,COLNM,COLORD,COLDESC,ADDDT,MODDT
          from BC_TBLINFO where TBLNM like #F_TBLNM#
          order by COLORD ASC
          ";
    ...
    $colord = "TBLNM,COLID,COLNM,COLORD,COLDESC,ADDDT,MODDT";


    $sql_inserted = "
               insert into BC_TBLINFO (
                                    TBLNM,COLID,COLNM,COLORD,COLDESC
                                    ,ADDDT
               ) values (
                                    #TBLNM#,#COLID#,#COLNM#,#COLORD#,#COLDESC#
    ...
    ";
    $sql_inserted_coltype = "sssis";

    $sql_deleted = " delete from BC_TBLINFO where TBLNM = #TBLNM# and COLID = #COLID#  ";
    $sql_deleted_coltype = "ss";

    $sql_updated = "
              update BC_TBLINFO set
                    COLNM = #COLNM#, COLORD = #COLORD#, COLDESC = #COLDESC#
                    ,MODDT = date_format(sysdate(),'%Y%m%d%H%i%s')
               where TBLNM = #TBLNM# and COLID = #COLID#
    ";
    $sql_updated_coltype = "sisss";
