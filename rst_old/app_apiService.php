...
//include_once('App_apiInterface.php');
include_once('app_apiDao.php');
//class App_apiService implements App_apiInterface
class app_apiService 
{
	private $DAO;
	private $DB;
	//생성자
	function __construct(){
		alog("App_apiService-__construct");

		$this->DAO = new app_apiDao();
	    //$this->DB = db_s_open();
		$this->DB["DATING"] = db_obj_open(getDbSvrInfo("DATING"));
	}
	//파괴자
	function __destruct(){
		alog("App_apiService-__destruct");
	...
	function __toString(){
		alog("App_apiService-__toString");
	}
	//컨디션1, 저장
	public function goC2Save(){
		...
		alog("APP_APIService-goC2Save________________________start");
	...
	//암호화컬럼
		...
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("APP_APIService-goC2Save________________________end");
	}
	//그리드1, 조회
	public function goG3Search(){
		...
		alog("APP_APIService-goG3Search________________________start");
		//그리드 서버 조회 
	...
	//암호화컬럼
		...
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("APP_APIService-goG3Search________________________end");
	}
	//그리드1, 완전삭제
	public function goG3Chksave(){
		...
		alog("APP_APIService-goG3Chksave________________________start");
		...
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("APP_APIService-goG3Chksave________________________end");
	}
	//그리드1, S
	public function goG3Save(){
		...
		alog("APP_APIService-goG3Save________________________start");
		//GRID_SAVE____________________________start
		$grpId="G3";
		$GRID["XML"]=$REQ[$grpId."-XML"];
		$GRID["COLORD"] = "ROWCHK,API_SEQ,API_NM,PGM_ID,URL,REQ_ENCTYPE,REQ_DATATYPE,REQ_BODY,RES_BODY,MYFILE,MYFILESVRNM,ADD_DT,MOD_DT,CHK"; //그리드 컬럼순서(Hidden컬럼포함)
	//암호화컬럼
		...
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("APP_APIService-goG3Save________________________end");
	}
	//그리드1, E
	public function goG3Excel(){
		...
		alog("APP_APIService-goG3Excel________________________start");
		//처리 결과 리턴
		$rtnVal->RTN_CD = "200";
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("APP_APIService-goG3Excel________________________end");
	}
	//폼뷰1, 조회
	public function goF4Search(){
		...
		alog("APP_APIService-goF4Search________________________start");
//FORMVIEW SEARCH
	//암호화컬럼
		$FORMVIEW["COLCRYPT"] = array("REQ_BODY"=>"CRYPT","RES_BODY"=>"CRYPT");
// SQL LOOP
		...
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("APP_APIService-goF4Search________________________end");
	}
	//폼뷰1, 저장
	public function goF4Save(){
		...
		alog("APP_APIService-goF4Save________________________start");
		//FORMVIEW SAVE
		$grpId="F4";
		$FORMVIEW["FNCTYPE"] = $REQ[$grpId . "-CTLCUD"]; 
		$GRID["KEYCOLID"] = "";  //KEY컬럼 COLID, -1
	...
	//암호화컬럼
		...
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("APP_APIService-goF4Save________________________end");
	}
	//폼뷰1, 삭제
	public function goF4Delete(){
		...
		alog("APP_APIService-goF4Delete________________________start");
//FORMVIEW DELETE
		$grpId="F4";
		$FORMVIEW["FNCTYPE"] = $REQ[$grpId."-CTLCUD"]; 
		$FORMVIEW["SQL"][$FORMVIEW["FNCTYPE"]] = $this->DAO->delApi($REQ); 
		...
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("APP_APIService-goF4Delete________________________end");
	}
}
