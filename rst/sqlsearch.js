//글로벌 변수 선언	
//버틀 그룹쪽에서 컨틀롤러 호출
var url_G1_SEARCHALL = "sqlsearchController.php?CTLGRP=G1&CTLFNC=SEARCHALL";//버틀 그룹쪽에서 컨틀롤러 호출
var url_G1_SAVE = "sqlsearchController.php?CTLGRP=G1&CTLFNC=SAVE";//버틀 그룹쪽에서 컨틀롤러 호출
var url_G1_RESET = "sqlsearchController.php?CTLGRP=G1&CTLFNC=RESET";//조건 변수 선언	
...
//컨트롤러 경로
var url_G2_SEARCH = "sqlsearchController.php?CTLGRP=G2&CTLFNC=SEARCH";
//컨트롤러 경로
var url_G2_RELOAD = "sqlsearchController.php?CTLGRP=G2&CTLFNC=RELOAD";
//컨트롤러 경로
var url_G2_HIDDENCOL = "sqlsearchController.php?CTLGRP=G2&CTLFNC=HIDDENCOL";
//컨트롤러 경로
var url_G2_EXCEL = "sqlsearchController.php?CTLGRP=G2&CTLFNC=EXCEL";
//그리드 객체
var mygridG2,isToggleHiddenColG2,lastinputG2,lastinputG2json,lastrowidG2;
var lastselectG2json;//그리드 변수 초기화	
//컨트롤러 경로
var url_G3_SEARCH = "sqlsearchController.php?CTLGRP=G3&CTLFNC=SEARCH";
//컨트롤러 경로
var url_G3_RELOAD = "sqlsearchController.php?CTLGRP=G3&CTLFNC=RELOAD";
//컨트롤러 경로
var url_G3_HIDDENCOL = "sqlsearchController.php?CTLGRP=G3&CTLFNC=HIDDENCOL";
//컨트롤러 경로
var url_G3_EXCEL = "sqlsearchController.php?CTLGRP=G3&CTLFNC=EXCEL";
//그리드 객체
var mygridG3,isToggleHiddenColG3,lastinputG3,lastinputG3json,lastrowidG3;
var lastselectG3json;//디테일 변수 초기화	

var obj_G4_SQLID_valid = jQuery.parseJSON( '{ "G4_SQLID": {"REQUARED":"",  "MIN":"",  "MAX":"",  "DATASIZE":30,  "DATATYPE":"STRING"} }' );   // SQLID 밸리데이션 선언
var obj_G4_SQLTXT_valid = jQuery.parseJSON( '{ "G4_SQLTXT": {"REQUARED":"",  "MIN":"",  "MAX":"",  "DATASIZE":1000,  "DATATYPE":"STRING"} }' );   // SQLTXT 밸리데이션 선언
//폼뷰 컨트롤러 경로
var url_G4_SEARCH = "sqlsearchController.php?CTLGRP=G4&CTLFNC=SEARCH";
//폼뷰 컨트롤러 경로
var url_G4_RELOAD = "sqlsearchController.php?CTLGRP=G4&CTLFNC=RELOAD";
  ...
  alog("G2_INIT()-------------------------start");
	...
	//onCheck
		...
		mygridG2.attachEvent("onRowSelect",function(rowID,celInd){
			...
			//', "PGMNM" : "' + q(mygridG2.cells(rowID,mygridG2.getColIndexById("PGMNM")).getValue()) + '"' +
		...
		//A124
			...
			', "PJTSEQ" : "' + q(mygridG2.cells(rowID,mygridG2.getColIndexById("PJTSEQ")).getValue()) + '"' +
			'}');
		G3_SEARCH(lastinputG3); //자식그룹 호출 : SQL
		});
  ...
  alog("G3_INIT()-------------------------start");
	...
	//onCheck
		...
		mygridG3.attachEvent("onRowSelect",function(rowID,celInd){
			...
			//', "SQLSEQ" : "' + q(mygridG3.cells(rowID,mygridG3.getColIndexById("SQLSEQ")).getValue()) + '"' +
			//', "SQLID" : "' + q(mygridG3.cells(rowID,mygridG3.getColIndexById("SQLID")).getValue()) + '"' +
			//', "SQLNM" : "' + q(mygridG3.cells(rowID,mygridG3.getColIndexById("SQLNM")).getValue()) + '"' +
			//', "CRUD" : "' + q(mygridG3.cells(rowID,mygridG3.getColIndexById("CRUD")).getValue()) + '"' +
			//', "RTN_TYPE" : "' + q(mygridG3.cells(rowID,mygridG3.getColIndexById("RTN_TYPE")).getValue()) + '"' +
		...
		//A124
			...
			', "PJTSEQ" : "' + q(mygridG3.cells(rowID,mygridG3.getColIndexById("PJTSEQ")).getValue()) + '"' +
			'}');
		G4_SEARCH(lastinputG4); //자식그룹 호출 : 폼
		});
...
//컬럼 초기화
	//SQLID, SQLID 초기화	  alog("G4_INIT()-------------------------end");
}
//D146 그룹별 기능 함수 출력		
// CONDITIONSearch	
    ...
    function G2_SEARCH(tinput){
        ...
        $.ajax({
            ...
            error: function(error){
                ...
                alog("[프로그램] Ajax http 500 error ( " + error + " )");
            }
        });
