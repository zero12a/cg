...
include_once('app_apiService.php');
...
alog("App_apiControl___________________________start");
...
}else if($objAuth->isAuth("APP_API",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"APP_API",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"APP_API",$ctl,"N");
...
$REQ["F4-API_SEQ"] = getFilter($REQ["F4-API_SEQ"],"REGEXMAT","/^[0-9]{1}[0-9]*$/");	
...
$REQ["F4-ADD_DT"] = getFilter($REQ["F4-ADD_DT"],"REGEXMAT","/^[0-9]{1}[0-9]*$/");	
...
alog("App_apiControl___________________________end");
