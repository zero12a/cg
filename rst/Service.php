...
class Service 
{
	...
	public function goC2Save(){
		...
		alog("Service-goC2Save________________________start");
	...
	//암호화컬럼
		...
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("Service-goC2Save________________________end");
	}
	//그리드1, 조회
	public function goG3Search(){
		...
		alog("Service-goG3Search________________________start");
		//그리드 서버 조회 
	...
	//암호화컬럼
		...
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("Service-goG3Search________________________end");
	}
	//그리드1, 완전삭제
	public function goG3Chksave(){
		...
		alog("Service-goG3Chksave________________________start");
		...
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("Service-goG3Chksave________________________end");
	}
	//그리드1, S
	public function goG3Save(){
		...
		alog("Service-goG3Save________________________start");
		//GRID_SAVE____________________________start
		$grpId="G3";
		$GRID["XML"]=$REQ[$grpId."-XML"];
		$GRID["COLORD"] = "ROWCHK,API_SEQ,API_NM,PGM_ID,URL,REQ_ENCTYPE,REQ_DATATYPE,REQ_BODY,RES_BODY,MYFILE,MYFILESVRNM,ADD_DT,MOD_DT,CHK"; //그리드 컬럼순서(Hidden컬럼포함)
	//암호화컬럼
		...
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("Service-goG3Save________________________end");
	}
	//그리드1, E
	public function goG3Excel(){
		...
		alog("Service-goG3Excel________________________start");
		//처리 결과 리턴
		$rtnVal->RTN_CD = "200";
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("Service-goG3Excel________________________end");
	}
	//폼뷰1, 조회
	public function goF4Search(){
		...
		alog("Service-goF4Search________________________start");
//FORMVIEW SEARCH
	//암호화컬럼
		$FORMVIEW["COLCRYPT"] = array("REQ_BODY"=>"CRYPT","RES_BODY"=>"CRYPT");
// SQL LOOP
		...
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("Service-goF4Search________________________end");
	}
	//폼뷰1, 저장
	public function goF4Save(){
		...
		alog("Service-goF4Save________________________start");
		//FORMVIEW SAVE
		$grpId="F4";
		$FORMVIEW["FNCTYPE"] = $REQ[$grpId . "-CTLCUD"]; 
		$GRID["KEYCOLID"] = "";  //KEY컬럼 COLID, -1
	...
	//암호화컬럼
		...
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("Service-goF4Save________________________end");
	}
	//폼뷰1, 삭제
	public function goF4Delete(){
		...
		alog("Service-goF4Delete________________________start");
//FORMVIEW DELETE
		$grpId="F4";
		$FORMVIEW["FNCTYPE"] = $REQ[$grpId."-CTLCUD"]; 
		$FORMVIEW["SQL"][$FORMVIEW["FNCTYPE"]] = $this->DAO->delApi($REQ); 
		...
		$rtnVal->ERR_CD = "200";
		echo json_encode($rtnVal);
		alog("Service-goF4Delete________________________end");
	}
}
